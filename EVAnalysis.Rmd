...
My client is an industry-leading energy infrastructure provider based in Europe. Following onto the energy crisis that Europe is facing (Popkostova, 2022), they are in the process of securing more funding from investors, and to invest in energy projects in the Europe to meet the ever-growing electricity demand.

One of their teams that focuses on renewable energy projects is eager to discover insights from the EV market. They would like to know if EV market is impacting the electricity demand in the Europe and by what magnitude, so that data-driven decisions could be made based on conclusions drawn from this data analysis project. Once we produce actionable recommendations to them, they could then conduct feasibility study based on our conclusion and advise their board.
...
We would then draw conclusions from the analysis steps above and advise stakeholders based on discovered insights to make actionable data-driven recommendations.
...
head(EVdemand)
```
The EV sales dataframe is grouped by countries(denoted by region) and vehicle type (denoted by powertrain), and value representing EV sales annually.
The EV chargers dataframe is grouped by countries(denoted by region) and charger types(denoted by powertrain), and value representing available public chargers in that specific year.
The EV demand dataframe is grouped by countries(denoted by region), and their corresponding annual electricity consumption is categorized by vehicle type (denoted by powertrain).
...
* The default datatype assigned by R could be inappropriate
...
### Procedures of our data cleaning process:
